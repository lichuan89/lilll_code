...
def conv_gradient(X, output_grad, W, b, stride=1):
    ...
    log('prepare to calc JX.')
    expand_output_grad_col = img_2_col(expand_output_grad, W.shape, stride)   
    log('begin to calc JX. grand_col:', expand_output_grad.shape, expand_output_grad_col.shape)
...
def avgpool(x, ksize=(2, 2), stride=2):
    """
    x是4维图片: num, row, col, channel
    """
    ...
    for r in range(0, row, stride):
        for c in range(0, col, stride):
            ...
            out[:, int(math.floor(1.0 * r / stride)), int(math.floor(1.0 * c / stride)), :] = np.sum(region, axis=1) / m
            index[:, r: rend, c: cend, :] += 1.0 / m # 1/m的梯度反向传播
    log('finish to calc maxpool:', x.shape, ksize)
    return out, index


def avgpool_gradient(grad, index, ksize=(2, 2), stride=1):
