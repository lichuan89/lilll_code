...
def collect_image_train_data(files, expand_num=10, use_hist=False):
    mats = None
    data = None
    T = None
    idx = 0
    for fname in files:
        ...
        imgs = [image]
        if files[fname] != '0': 
            imgs = imgs + [expand_image(image) for i in range(expand_num)]
    ...
    if False:
        ...
        if os.path.exists('%s.fea' % train_image_fpath):
            ...
            for line in open(train_image_fpath).readlines():
                ...
                files[fpath] = tag
            data, T = collect_image_train_data(files, expand_num=2)
